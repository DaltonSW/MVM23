...
func _draw() -> void:
    ...
    var player_position_center := Vector2(area.x / 2, area.y / 2) * MetSys.CELL_SIZE + Vector2(12, 12)

    var player_dot_size := 2.0 # Size of the dot, adjust as needed
    var player_dot_color := Color(1.0, 0, 0) # Red color for the player dot, change as desired
    draw_circle(player_position_center, player_dot_size, player_dot_color)
